...
```text
Visual Studio 项目: 【L01_DemoRunner】
└── 📁 源文件
    └── 📄 menu_runner.c  <--- ⭐ 整个项目只需添加这 1 个文件！
```

#### 📌 操作步骤：
1. 在 Visual Studio 中新建一个空的控制台应用程序（如 `L01_Hello`）；
2. 在右侧 **解决方案资源管理器 $\rightarrow$ 源文件** 中，**【只需添加 `lecture-demo/menu_runner.c` 这一个文件】**；
3. 直接按下 **`Ctrl + F5`**（开始执行不调试）；
4. 终端会立即弹出精美的数字交互菜单，输入 `0~5` 即可随意挑选执行 1.0 ~ 1.5 全部 6 个案例，演示完毕后按回车键即可返回主菜单！

---

### 2. 🚨【MSVC 安全检查排雷】`error C4996: 'scanf' may be unsafe`

在 VS 中调用标准 `scanf` 时，微软编译器默认会抛出 `C4996` 错误拦截生成。

#### 💡 解决方案：
- **方案 A（源码级，全库已配置）**：确保源文件最顶部第 1 行写有 `#define _CRT_SECURE_NO_WARNINGS`（必须在 `#include <stdio.h>` 之前）；
- **方案 B（项目全局级，一劳永逸）**：
  1. 右键项目 $\rightarrow$ **属性 (Properties)**；
  2. 点击 **配置属性 $\rightarrow$ C/C++ $\rightarrow$ 预处理器 (Preprocessor)**；
  3. 在 **预处理器定义** 中添加：`_CRT_SECURE_NO_WARNINGS` $\rightarrow$ 点击确定。

---

### 3. 🎯【学生单题练习 SOP】同一个项目多个 `.c` 文件如何切换？

当学生在同一个项目中保留多个练习源文件时，可使用以下两种标准操作：

#### 方案 A：【从生成中排除（Excluded From Build）】⭐⭐⭐⭐⭐（首选，零侵入代码）
1. 在右侧 **解决方案资源管理器** 中，找到当前**不打算运行的 `.c` 文件**；
2. **右键点击该文件** $\rightarrow$ 选择 **属性（Properties）**；
3. 在 **配置属性 $\rightarrow$ 常规** 中，将 **【从生成中排除】（Excluded From Build）** 设为 **【是 (Yes)】** $\rightarrow$ 点击确定；
4. 按 **`Ctrl + F5`** 即可单独编译运行目标文件！下次想切换时改回【否】即可。

#### 方案 B：【多项目独立管理（设为启动项目）】⭐⭐⭐⭐（工程化结构）
- 在解决方案下为每个案例新建独立项目（如 `Task1_Hello`、`Task2_Card`）；
- 想运行哪个案例，只需**右键点击对应项目 $\rightarrow$ 选择【设为启动项目（Set as Startup Project）】**，按 `Ctrl + F5` 直接运行。
