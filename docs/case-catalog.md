...
### 案例 1-0：虚拟 7 条 CPU 指令模拟器
| 字段 | 内容 |
|------|------|
| 讲次与案例名称 | L01-00 极简虚拟 CPU 指令系统模拟器 |
| 教学目标 | 揭秘“程序=指令的有序组合”物理本质，模拟 A+B+C 顺序计算与 A*B 循环累加跳转 |
| 输入 | 无（内置数据模拟） |
| 核心处理 | 模拟 Input/Output/Add/Set/BranchEq/Jump 7 条指令调度，展示寄存器数据流变 |
| 输出 | 逐步打印指令执行轨迹与单元数值状态 |
| 关键知识点 | 指令、指令系统、程序、顺序执行、条件跳转与循环启蒙 |
| 测试正常值 | A+B+C 输出 16，A*B 输出 24 |
| 对应源文件 | [`lessons/L01_intro/lecture-demo/1.0_instruction_simulator.c`](../lessons/L01_intro/lecture-demo/1.0_instruction_simulator.c) |
| 对应PPT页码 | 第10~12页 |
| 当前状态 | **已定稿** |
...
| 字段 | 内容 |
|------|------|
| 讲次与案例名称 | L01-01 Hello World 最简标准 C 结构 |
| 教学目标 | 识别C程序基本结构与四大基石（头文件、main函数、函数体、return 0） |
| 输入 | 无 |
| 核心处理 | printf 输出固定文本 |
| 输出 | `Hello, C!\n` |
| 关键知识点 | #include <stdio.h>、int main(void)、printf、return 0、英文分号 |
| 测试正常值 | 正常编译并输出 `Hello, C!` |
| 测试边界值 | 英文半角标点校验 |
| 测试特殊值/错误输入 | 漏写分号、漏写花括号 |
| 常见错误 | 忘记 `#include <stdio.h>`、main 写成 `mian`、语句末尾无分号 |
| 对应源文件 | [`lessons/L01_intro/lecture-demo/1.1_hello_world.c`](../lessons/L01_intro/lecture-demo/1.1_hello_world.c) |
| 对应PPT页码 | 第14~15页 |
| 当前状态 | **已定稿** |
...
| 字段 | 内容 |
|------|------|
| 讲次与案例名称 | L01-02 多行文本输出与转义换行 |
| 教学目标 | 理解 `\n` 换行符机制和 printf 顺序执行逻辑，辨析 `\n` 与 `/n` |
| 输入 | 无 |
| 核心处理 | 多个 printf 语句自上而下顺序执行，`\n` 开启新行 |
| 输出 | 多行文本清晰排版 |
| 关键知识点 | `\n` 转义字符、printf 顺序执行逻辑 |
| 测试正常值 | 输出 3 行文本并换行 |
| 常见错误 | 忘记 `\n` 导致输出连在一行、写反为 `/n` |
| 对应源文件 | [`lessons/L01_intro/lecture-demo/1.2_multi_line_output.c`](../lessons/L01_intro/lecture-demo/1.2_multi_line_output.c) |
| 对应PPT页码 | 第17页 |
| 当前状态 | **已定稿** |
...
| 字段 | 内容 |
|------|------|
| 讲次与案例名称 | L01-05 求两数之和（谭浩强例1.2） |
| 教学目标 | 掌握变量定义（先定义后使用）、赋值运算符 `=` 及 `%d` 整型格式化占位符 |
| 输入 | 无（代码赋初值 a=123, b=456） |
| 核心处理 | 计算 `sum = a + b` |
| 输出 | `sum is 579\n` |
| 关键知识点 | `int` 整型变量、`=` 赋值号、算术加法表达式、`%d` 占位符 |
| 测试正常值 | 输出 `sum is 579` |
| 对应源文件 | [`lessons/L01_intro/lecture-demo/1.5_sum_two_numbers.c`](../lessons/L01_intro/lecture-demo/1.5_sum_two_numbers.c) |
| 对应PPT页码 | 第16页 |
| 当前状态 | **已定稿** |
