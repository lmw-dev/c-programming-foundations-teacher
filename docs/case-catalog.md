...
### 案例 16-1：升本模拟综合练习与单链表降序构建持久化
| 字段 | 内容 |
|------|------|
| 讲次与案例名称 | L16-01 升本模拟与单链表降序排行榜 |
| 教学目标 | 全课程十章核心知识串联、单链表按成绩降序动态构建与持久化落盘 |
| 输入 | 5 名学生的成绩记录（无序动态插入） |
| 核心处理 | malloc 动态分配节点 → insert_sorted 降序有序插入 → save_list_to_file 文件落盘 → free_list 释放内存 |
| 输出 | 控制台格式化排行榜单及 sorted_result.txt 文件 |
| 关键知识点 | malloc/free、套娃指针next、单链表有序插入、指针时序、作用域生命期、文件流 |
| 测试正常值 | 5 名学生无序插入后输出完整降序榜单 |
